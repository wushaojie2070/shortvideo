				...
				uni.chooseImage({
					...
					success: (resp) => {
						...
						uni.uploadFile({
							...
							success: (res) => {
								console.log("成功了",res.data.split(","))
								this.bgImg = "http://skrvideo.fun:9000" + res.data.split(",")[15].slice(30,-1);
								let pages = getCurrentPages(); //获取所有页面栈实例列表
								let prevPage = pages[pages.length - 2]; //上一页页面实例
								if (pages.length == 2) {
									prevPage.$vm.$children[0].userInfo.bgImg = "http://skrvideo.fun:9000" + res.data.split(",")[15].slice(30,-1);
								} else if (pages.length == 3) {
									prevPage.$vm.userInfo.bgImg = "http://skrvideo.fun:9000" + res.data.split(",")[15].slice(30,-1);
								}
								uni.showToast({
									icon: "none",
									title: "修改成功！"
